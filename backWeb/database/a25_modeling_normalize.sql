...
경우가 나타난다.
2. 게시물에 답글을 계층적으로 달아야 하는 경우, 직급에 있어서 상위
관리자와 하위 관리자를 구분하여 처리하는 경우, 메뉴 항목에 있어서
상위메뉴와 하위 메뉴를 처리하여야 하는 경우 등 여러가지 경우가 발생한다.
...
4. 계층적 질의문의 위한 테이블 구조
	1) 계층적 질의문 처리를 위헤서는, 테이블의 특정 컬럼의
	계층구조로 되어있어야 한다.
	우리가 많이 아는 emp테이블의 empno와 mgr이 전형적인 계층적
...
5. 계층적 질의문의 기본 형식
	...
	4) 사용코드
		...
		level : 계층레벨을 가져온다. 위 공과대학은 1level
			멀티미디어학과는 3level을 가져온다.
		조건1 : 계층구조가 시작하는 고전을 말한다. 테이블의
